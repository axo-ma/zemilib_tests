...
''', stop_on_error=stop))
    ...
    def test_atomic_report_is_updated_and_markdown_reports_are_linked(self) -> None:
        ...
        main = (component.run_directory / "index.md").read_text(encoding="utf-8")
        self.assertIn("# Job Report", main)
        self.assertIn("## Module Summary", main)
        self.assertNotIn("## Results", main)
        self.assertNotIn(dangerous, main)
        self.assertFalse((component.run_directory / "report.html").exists())
        self.assertFalse((component.run_directory / "main.md").exists())
        self.assertFalse((component.run_directory / "report.md").exists())
        ...
        for playbook in component.playbooks:
            ...
            self.assertIn("&lt;/script&gt;", module_markdown)
            self.assertNotIn(dangerous, module_markdown)
            self.assertTrue(component.reporting.writer.ref("run", playbook.module_id, playbook.trial_id))
